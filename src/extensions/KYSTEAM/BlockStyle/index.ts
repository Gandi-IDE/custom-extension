...
export default class BlockStyle extends GandiExtension {
  ...
  initConfig(extConfigHost) {
    const catInfos = this.getJSON(
      extConfigHost + '/config/cat-config.json'
    );
    const tagInfos = this.getJSON(
      extConfigHost + '/config/tag-config.json'
    );
    const menuInfos = this.getJSON(
      extConfigHost + '/config/menu-config.json'
    );
  ...
  refreshWorkspace() {
    ...
    Blockly.BlockSvg.prototype.initCatStuff = function () {
      ...
      if (catInfo.use_asset_v2) {
        ...
        if (catInfo.isCustom) {
          this.userPic1.setAttribute('x', '95');
          this.userPic1.setAttribute('y', '-25');
          this.userPic1.style.height = '19px';
          // this.userPic1.style.width = '100px';
          // this.userPic1.style.objectFit = 'cover';
        } else {
          this.userPic1.setAttribute('x', '-1');
          this.userPic1.setAttribute('y', '-33');
        }
    ...
    if (workspace) {
      ...
      if (flyout) {
        ...
        this.Blockly.Xml.clearWorkspaceAndLoadFromXml(
          this.Blockly.Xml.workspaceToDom(flyoutWorkspace),
          flyoutWorkspace
        );
        // 刷新工具盒子
  ...
  buildCatMenu() {
    ...
    this.menuList.forEach(sub => {
      const tag = this.tagInfos[sub.type] ?? sub.type;
      catMenuList.push({ text: `☁ [${tag}] ${sub.text}`, value: sub.value });
    });
    return catMenuList;
  }
  ...
  setCurrentStyle(args) {
    ...
    setTimeout(() => {
      // 延迟 1s，让积木高亮完毕
      // TODO: 优化-使用 runtime.glowBlock(id, isGlowing) 来取消高亮
      // TODO: debounce 或者 限制执行频率
      this.refreshWorkspace();
    }, 1000);
