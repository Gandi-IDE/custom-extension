...
var DannyDevCOM = { 'wsrecv_': [], 'wsnotalive_': [], 'json_list': [], 'wsock': [], 'http_pm_': [] }
export default class DannyDEVCOMM extends GandiExtension {
  ...
  init() {
    ...
    http.setArguments({ g_way, site, p, body,after })
    this.addBlock(http)
  ...
  get_p(p) {
    ...
    for (let i = 0, len = DannyDevCOM.http_pm_.length; i < len; i++) {
      if (DannyDevCOM.http_pm_[i]['id'] == String(p)) {
        for (let i_ = 0, len_ = DannyDevCOM.http_pm_[i_].p.length; i_ < len_; i_++) {
          if (i_ == 0) {
  ...
  http(args) {
    let ret=''
    const { g_way, site, p, body,after } = args
    if (p == 'None') {
       ret =this.httpGet(site, body, g_way);
    }
    else {
       ret =this.httpGet(site + this.get_p(p), body, g_way);
    }
    ...
    for (let i = 0, len = DannyDevCOM.wsock.length; i < len; i++) {
      if (String(id) == DannyDevCOM.wsock[i]["id"]) {
        ...
        try {
          ...
          DannyDevCOM.wsock[i]['obj'].onclose = function (evt) {
            DannyDevCOM.wsnotalive_.push(DannyDevCOM.wsock[i]['id'])
          }
          DannyDevCOM.wsock[i]['obj'].onopen = function () {
            status = true
        }
        DannyDevCOM.wsock[i]['obj'].onerror = function(event) {
            status=true
          };
    }
