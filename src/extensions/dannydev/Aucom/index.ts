...
export default class DannyDEVCOMM extends GandiExtension {
  ...
  setting = {
    http_cache: true //http访问cache加速和减小服务器压力 如果服务器没有防御/带宽过小/日常服务器压力过大 建议开启
  }
  ...
  init() {
    ...
    let data = BlockUtil.createArgument(ArgumentType.STRING, ' ')
    ...
    const urlencoding = BlockUtil.createReporter();
    urlencoding.setReporterScope(ReporterScope.TARGET)
    urlencoding.setOpcode('urlencoding')
    urlencoding.setText('DannyDevCOM.code_url_en')
    TEXT = BlockUtil.createArgument(ArgumentType.STRING, 'abcd')
    urlencoding.setArguments({ key, TEXT })
    this.addBlock(urlencoding)

    const urldecoding = BlockUtil.createReporter();
    urldecoding.setReporterScope(ReporterScope.TARGET)
    urldecoding.setOpcode('urldecoding')
    urldecoding.setText('DannyDevCOM.code_url_de')
    TEXT = BlockUtil.createArgument(ArgumentType.STRING, 'abcd')
    urldecoding.setArguments({ key, TEXT })
    this.addBlock(urldecoding)
    ...
    const ecallback = BlockUtil.createHat();
    ecallback.setOpcode('errorcallback')
    ecallback.setText('DannyDevCOM.ecallback')
    //this.addBlock(ecallback)
  ...
  httpGet(theUrl, body, method, header) {
    if (this.setting.http_cache) {
      return this.xhrcache.xhr(theUrl,method,body,header)
    }
    try {
  ...
  ws_cre_sock(args) {
    const { yi } = args
    if (yi == 'WS') {
      let id = String(Date.now() * Math.random() % Math.random() + Math.random() + DannyDevCOM.wsock.length)
  ...
  async urlencoding(args) {
    const { TEXT } = args
    return encodeURIComponent(String(TEXT)); // 编码
  }
  async urldecoding(args) {
    const { TEXT } = args
    try {
      return decodeURIComponent(String(TEXT)); // 解码
    }
    catch (error) {
      return error
    }
  ...
  json_create(args) {
    const { json_ } = args
    let j = json_
    let id = String(Date.now() * Math.random() % Math.random() + Math.random() + DannyDevCOM.json_list.length)
    let a = {}
    try {
      a = JSON.parse(j)
    }
  ...
  async recv_socket(args) {
    ...
    if (_ret['code'] == 200) {
      try {
        return JSON.stringify(JSON.parse(_ret['msg']))
      }
      catch (error) {
        return JSON.stringify(_ret['msg'])
      }
    }
    return ''
  }
  async clsconn_socket(args) {
    ...
    await this._wsget('ws://127.0.0.1:23089/socket/close_conn', { id, ip: String(ip) + ':' + String(port) }).then(function (ret) {
      _ret = JSON.parse(ret)
    })
    return _ret['code'] == 200
  }
...
class xhrcache {
  ...
  constructor() {
      ...
      setInterval(() => {
        let l = Object.keys(this.cl)
        for (let i = 0, len = l.length; i < len; i++) {
          if (this.timeFn(this.cl[l[i]][1])[1] <= 30) { }
          else {
            Reflect.deleteProperty(this.cl, l[i])
          }
  ...
  async decoding(TEXT) {
    let ALPHABET = 'Q!@#$%&*(O)P8A-S_D+F=GHJKL'
    let ALPHABET_MAP = {};
    let BASE = ALPHABET.length;
    for (let i = 0; i < ALPHABET.length; i++) {
      ALPHABET_MAP[ALPHABET.charAt(i)] = i;
  ...
  xhr(theUrl: string, method, body, header) {
    try{
    console.log(String(this.encoding(theUrl.split('//', 2)[1])),theUrl.split(':', 2))
      if (String(this.encoding(theUrl.split('//', 2)[1])) in this.cl) {
        if (this.timeFn(this.cl[String(this.encoding(theUrl.split('//', 2)[1]))][1])[1] <= 30) {
          return this.decoding(this.cl[String(this.encoding(theUrl.split('//', 2)[1]))][0])
        }
        else {
          Reflect.deleteProperty(this.cl, String(this.encoding(theUrl.split('//', 2)[1])))
        }
      }
      let xmlHttp = new XMLHttpRequest();
      xmlHttp.open(method, theUrl, false); // false 为同步请求 true 异步请求
      ...
      this.cl[String(this.encoding(theUrl.split('//', 2)[1]))] = [this.encoding(res), new Date()]
      console.log(this.cl)
      return res;}
      catch(error){
        return error
