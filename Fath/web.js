class web {
  constructor(runtime) {
    this.runtime = runtime
    this._formatMessage = runtime.getFormatMessage({
      'zh-cn': {
        'WebExt.extensionName': "Fath's网络",
        'WebExt.fetch': '获取[URL]',
        'WebExt.redirect': '重定向至 [LINK]',
      },

      en: {
        'WebExt.extensionName': "Fath's Web",
        'WebExt.fetch': 'Fetch [URL]',
        'WebExt.redirect': 'Redirect to [LINK]',
      },
  })
}

formatMessage(id) {
  return this._formatMessage({
    id,
    default: id,
    description: id,
  })
}
  
  getInfo() {
    return {
      id: "fathweb",
      name: this.formatMessage('WebExt.extensionName'),
      color1: "#C23373",
      color2: "#C23373",
      color3: "#C23373",
      blockIconURI: "data:image/png;base64,",
      blocks: [
        {
          opcode: "fetch",
          blockType: Scratch.BlockType.REPORTER,
          text: this.formatMessage('WebExt.fetch'),
          arguments: {
            URL: {
              type: Scratch.ArgumentType.STRING,
              defaultValue: "https://random-word-api.herokuapp.com/word",
            },
          },
        },
        {
          opcode: "redirect",
          blockType: Scratch.BlockType.COMMAND,
          text: this.formatMessage('WebExt.redirect'),
          arguments: {
            LINK: {
              type: Scratch.ArgumentType.STRING,
              defaultValue: "https://fathdev.fath11.repl.co/",
            },
          },
        },
      ],
    };
  }

  fetch(args) {
    return fetch(args.URL)
      .then((r) => r.text())
      .catch(() => "");
  }
  redirect(args) {
    Scratch.redirect(args.LINK);
  }
}

window.tempExt = {
  Extension: web,
  info: {
    name: "Fath's Web",
    description: 'Fetch, redirect, and more coming soon!',
    extensionId: 'fathweb',
    iconURL: "Fath/assets/fathweb.png",
    insetIconURL: "Fath/banners/Web.png",
    featured: true,
    disabled: false,
    collaborator: 'Fath11',
  },
  l10n: {
    'zh-cn': {
      'Web.extensionName': "Fath's网络",
      'Web.description': '获取、重定向等功能即将推出',
    },
    en: {
      'Web.extensionName': "Fath's Web",
      'Web.description': 'Fetch, redirect, and more coming soon!',
    },
  },
}