...
//Some variables
//t=Time(frame or seconds after started)
//b=Beginning(initial number)
//c=Delta(change in the number, final value is Beginning+Delta)
//d=Duration(the duration of the animation, in frames, or maybe seconds)
...
//Tweaks to make it nice and short
function Ani_Tweaks(menu, ini_t, val_1, val_2, duration) {
    //if the ini_t input is greater than duration, then we think that the user is trying to calculate ini_t=duration.
    if (ini_t > duration) {
        ini_t = duration;
    }
    switch (menu) {
        case 'easeInQuad':
            return Number(easeInQuad(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeOutQuad':
            return Number(easeOutQuad(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInOutQuad':
            return Number(easeInOutQuad(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInSine':
            return Number(easeInSine(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeOutSine':
            return Number(easeOutSine(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInOutSine':
            return Number(easeInOutSine(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInExpo':
            return Number(easeInExpo(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeOutExpo':
            return Number(easeOutExpo(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInOutExpo':
            return Number(easeInOutExpo(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInCirc':
            return Number(easeInCirc(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeOutCirc':
            return Number(easeOutCirc(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInOutCirc':
            return Number(easeInOutCirc(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInCubic':
            return Number(easeInCubic(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeOutCubic':
            return Number(easeOutCubic(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInOutCubic':
            return Number(easeInOutCubic(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInQuart':
            return Number(easeInQuart(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeOutQuart':
            return Number(easeOutQuart(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInOutQuart':
            return Number(easeInOutQuart(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInQuint':
            return Number(easeInQuint(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeOutQuint':
            return Number(easeOutQuint(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInOutQuint':
            return Number(easeInOutQuint(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInElastic':
            return Number(easeInElastic(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeOutElastic':
            return Number(easeOutElastic(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInOutElastic':
            return Number(easeInOutElastic(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInBack':
            return Number(easeInBack(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeOutBack':
            return Number(easeOutBack(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInOutBack':
            return Number(easeInOutBack(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInBounce':
            return Number(easeInBounce(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeOutBounce':
            return Number(easeOutBounce(ini_t, val_1, val_2, duration).toFixed(2));
        case 'easeInOutBounce':
            return Number(easeInOutBounce(ini_t, val_1, val_2, duration).toFixed(2));
        default:
            return null;
    }
}
...
class Animator {
    constructor(runtime) {
        ...
        this._formatMessage = runtime.getFormatMessage({
            'zh-cn': {
                ...
                'Ani.Animate': '以[menu]方式计算[ini_t]秒时[val_1]到[val_2]的值，持续时间为[duration]秒',//block
                'Ani.Move': '以[menu]方式移到[ini_t]秒时从[x_1][y_1]到[x_2][y_2]，持续时间[duration]秒',//block
                'Ani.easeInQuad': '二次缓入',
                ...
                'Ani.Fac_exp': '⚡测试功能（可能导致严重后果）',
                'Ani.aniGlide': '⚡以[menu]方式移到[x][y]，持续时间[duration]秒'
            },

            en: {
                'Ani.extensionName': "Animator",
                'Ani.Animate': 'Calculate the value between[val_1]and[val_2]using the[menu]method, with a duration of[duration]starting at[ini_t]',//block
                'Ani.Move': 'Move from[x_1][y_1]to[x_2][y_2]using the[menu]method, with a duration of[duration]starting at[ini_t]',//block
                ...
                'Ani.Fac_exp': '⚡Experimental features (may cause damage)',
                'Ani.aniGlide': '⚡Move to[x][y]using the[menu]method, with a duration of[duration]seconds'
            },
        })
    }
    ...
    getInfo() {
        return {
            ...
            blocks: [
                ...
                "---" + this.formatMessage("Ani.Fac_exp"),
                {
                    opcode: 'aniGlide',
                    blockType: 'command',
                    text: this.formatMessage('Ani.aniGlide'),
                    arguments:{
                        menu: {
                            type: "string",
                            menu: 'Ani',
    ...
    //value return
    //note that for advanced animation involving movements, resizing, effects, or directions, this block would be better than the Move() function.
    Animate(args) {
        if (args.duration != 0){
            let val_2 = args.val_2 - args.val_1;
            let ini_t = Math.abs(args.ini_t);
            let duration = Math.abs(args.duration);
            return Ani_Tweaks(args.menu, ini_t, args.val_1, val_2, duration);
        }
    }
    ...
    Move(args, util) {
        if (args.duration != 0) {
            let x_2 = args.x_2 - args.x_1;
            let y_2 = args.y_2 - args.y_1;
            let ini_t = Math.abs(args.ini_t);
            let duration = Math.abs(args.duration);

            util.target.setXY(Ani_Tweaks(args.menu, ini_t, args.x_1, x_2, duration), Ani_Tweaks(args.menu, ini_t, args.y_1, y_2, duration));
        } else {
            util.target.setXY(args.x_2, args.y_2);//bug fixed (maybe)
        }
    }

    //getEffect
    //note that this function can be replaced by Arkos' Extensions (@Arkos). It would be easier to use these blocks here compared to finding it in Arkos' Extensions.
    getEffect(args, util) {
        return util.target.effects[args.Effect];
    }

    //experimental blocks may have bugs (and may spoil the project!)
    //This function glides the target to the specified x and y coordinates over a specified duration.
    //This function is created with the help of @白猫 and @酷可, and many members of the CCW community.
    async aniGlide(args, util) {
        let startTime = 0; // Start time, set to 0
        let endTime = args.duration * 1000; // End time, i.e. specified time

        let interval = 30; // Move operation is executed every 100 milliseconds, can be adjusted as needed

        let currentTime = startTime; // Current time, initially set to start time

        let x_1 = util.target.x;
        let y_1 = util.target.y;
        ...
        return new Promise((resolve, reject) => {
            let timer = setInterval(function () {
                if (currentTime <= endTime) {
                    let progress = currentTime / endTime; // Calculate current time progress (between 0 and 1)

                    let newX = Ani_Tweaks(args.menu, currentTime, x_1, x_2, args.duration * 1000);
                    let newY = Ani_Tweaks(args.menu, currentTime, y_1, y_2, args.duration * 1000);
                    util.target.setXY(newX, newY);

                    currentTime += interval; // Increase current time
                } else {
                    clearInterval(timer); // Clear timer when end time is reached
                    resolve();
                }
            }, interval);
        });
