const ani_cover = "data:image/png;base64,";
const ani_icon = "data:image/svg+xml;base64,";

//This message is to inform developers about the version of this extension (used in dubuggings).
//note that after release, the console.clear() command should be deleted.
//console.clear();
console.log("Debug notes\nExt name: Animator\nExt version: v1.0.2\nExt developer: @官方小傲娇 (@DilemmaGX)");

//Animation Functions

//Some variables
//t=Time(frame or seconds after started)
//b=Beginning(initial number)
//c=Delta(change in the number, final value is Beginning+Delta)
//d=Duration(the duration of the animation, in frames, or maybe seconds)

function easeInQuad(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return c * (t / d) * (t / d) + b;
}
function easeOutQuad(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return -c * (t /= d) * (t - 2) + b;
}
function easeInOutQuad(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    if ((t /= d / 2) < 1) return c / 2 * t * t + b;
    return -c / 2 * ((--t) * (t - 2) - 1) + b;
}
function easeInSine(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return -c * Math.cos(t / d * (Math.PI / 2)) + c + b;
}
function easeOutSine(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return c * Math.sin(t / d * (Math.PI / 2)) + b;
}
function easeInOutSine(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return -c / 2 * (Math.cos(Math.PI * t / d) - 1) + b;
}
function easeInExpo(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return (t == 0) ? b : c * Math.pow(2, 10 * (t / d - 1)) + b;
}
function easeOutExpo(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return (t == d) ? b + c : c * (-Math.pow(2, -10 * t / d) + 1) + b;
}
function easeInOutExpo(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    if (t == 0) return b;
    if (t == d) return b + c;
    if ((t /= d / 2) < 1) return c / 2 * Math.pow(2, 10 * (t - 1)) + b;
    return c / 2 * (-Math.pow(2, -10 * --t) + 2) + b;
}
function easeInCirc(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return -c * (Math.sqrt(1 - (t /= d) * t) - 1) + b;
}
function easeOutCirc(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return c * Math.sqrt(1 - (t = t / d - 1) * t) + b;
}
function easeInOutCirc(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    if ((t /= d / 2) < 1) return -c / 2 * (Math.sqrt(1 - t * t) - 1) + b;
    return c / 2 * (Math.sqrt(1 - (t -= 2) * t) + 1) + b;
}
function easeInCubic(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return c * (t /= d) * t * t + b;
}
function easeOutCubic(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return c * ((t = t / d - 1) * t * t + 1) + b;
}
function easeInOutCubic(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    if ((t /= d / 2) < 1) return c / 2 * t * t * t + b;
    return c / 2 * ((t -= 2) * t * t + 2) + b;
}
function easeInQuart(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return c * (t /= d) * t * t * t + b;
}
function easeOutQuart(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return -c * ((t = t / d - 1) * t * t * t - 1) + b;
}
function easeInOutQuart(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    if ((t /= d / 2) < 1) return c / 2 * t * t * t * t + b;
    return -c / 2 * ((t -= 2) * t * t * t - 2) + b;
}
function easeInQuint(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return c * (t /= d) * t * t * t * t + b;
}
function easeOutQuint(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return c * ((t = t / d - 1) * t * t * t * t + 1) + b;
}
function easeInOutQuint(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    if ((t /= d / 2) < 1) return c / 2 * t * t * t * t * t + b;
    return c / 2 * ((t -= 2) * t * t * t * t + 2) + b;
}
function easeInElastic(t, b, c, d) {
    var s = 1.70158;
    var p = 0;
    var a = c;
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    if (t == 0) return b;
    if ((t /= d) == 1) return b + c;
    if (!p) p = d * .3;
    if (a < Math.abs(c)) {
        a = c;
        var s = p / 4;
    }
    else var s = p / (2 * Math.PI) * Math.asin(c / a);
    return -(a * Math.pow(2, 10 * (t -= 1)) * Math.sin((t * d - s) * (2 * Math.PI) / p)) + b;
}
function easeOutElastic(t, b, c, d) {
    var s = 1.70158;
    var p = 0;
    var a = c;
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    if (t == 0) return b;
    if ((t /= d) == 1) return b + c;
    if (!p) p = d * .3;
    if (a < Math.abs(c)) {
        a = c;
        var s = p / 4;
    }
    else var s = p / (2 * Math.PI) * Math.asin(c / a);
    return a * Math.pow(2, -10 * t) * Math.sin((t * d - s) * (2 * Math.PI) / p) + c + b;
}
function easeInOutElastic(t, b, c, d) {
    var s = 1.70158;
    var p = 0;
    var a = c;
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    if (t == 0) return b;
    if ((t /= d / 2) == 2) return b + c;
    if (!p) p = d * (.3 * 1.5);
    if (a < Math.abs(c)) {
        a = c;
        var s = p / 4;
    }
    else var s = p / (2 * Math.PI) * Math.asin(c / a);
    if (t < 1) return -.5 * (a * Math.pow(2, 10 * (t -= 1)) * Math.sin((t * d - s) * (2 * Math.PI) / p)) + b;
    return a * Math.pow(2, -10 * (t -= 1)) * Math.sin((t * d - s) * (2 * Math.PI) / p) * .5 + c + b;
}
function easeInBack(t, b, c, d) {
    var s = 1.70158;
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    if (s == undefined) s = 1.70158;
    return c * (t /= d) * t * ((s + 1) * t - s) + b;
}
function easeOutBack(t, b, c, d) {
    var s = 1.70158;
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    if (s == undefined) s = 1.70158;
    return c * ((t = t / d - 1) * t * ((s + 1) * t + s) + 1) + b;
}
function easeInOutBack(t, b, c, d) {
    var s = 1.70158;
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    if (s == undefined) s = 1.70158;
    if ((t /= d / 2) < 1) return c / 2 * (t * t * (((s *= (1.525)) + 1) * t - s)) + b;
    return c / 2 * ((t -= 2) * t * (((s *= (1.525)) + 1) * t + s) + 2) + b;
}
function easeInBounce(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    return c - easeOutBounce(d - t, 0, c, d) + b;
}
function easeOutBounce(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    if ((t /= d) < (1 / 2.75)) {
        return c * (7.5625 * t * t) + b;
    } else if (t < (2 / 2.75)) {
        return c * (7.5625 * (t -= (1.5 / 2.75)) * t + .75) + b;
    } else if (t < (2.5 / 2.75)) {
        return c * (7.5625 * (t -= (2.25 / 2.75)) * t + .9375) + b;
    } else {
        return c * (7.5625 * (t -= (2.625 / 2.75)) * t + .984375) + b;
    }
}
function easeInOutBounce(t, b, c, d) {
    t = Number(t);
    b = Number(b);
    c = Number(c);
    d = Number(d);
    if (t < d / 2) {
        return easeInBounce(t * 2, 0, c, d) * .5 + b;
    }
    return easeOutBounce(t * 2 - d, 0, c, d) * .5 + c * .5 + b;
}

//Tweaks to make it nice and short
function Ani_Tweaks(menu, ini_t, val_1, val_2, duration) {
    //if the ini_t input is greater than duration, then we think that the user is trying to calculate ini_t=duration.
    if (ini_t > duration) {
        ini_t = duration;
    }
    switch (menu) {
        case 'easeInQuad':
            return easeInQuad(ini_t, val_1, val_2, duration);
        case 'easeOutQuad':
            return easeOutQuad(ini_t, val_1, val_2, duration);
        case 'easeInOutQuad':
            return easeInOutQuad(ini_t, val_1, val_2, duration);
        case 'easeInSine':
            return easeInSine(ini_t, val_1, val_2, duration);
        case 'easeOutSine':
            return easeOutSine(ini_t, val_1, val_2, duration);
        case 'easeInOutSine':
            return easeInOutSine(ini_t, val_1, val_2, duration);
        case 'easeInExpo':
            return easeInExpo(ini_t, val_1, val_2, duration);
        case 'easeOutExpo':
            return easeOutExpo(ini_t, val_1, val_2, duration);
        case 'easeInOutExpo':
            return easeInOutExpo(ini_t, val_1, val_2, duration);
        case 'easeInCirc':
            return easeInCirc(ini_t, val_1, val_2, duration);
        case 'easeOutCirc':
            return easeOutCirc(ini_t, val_1, val_2, duration);
        case 'easeInOutCirc':
            return easeInOutCirc(ini_t, val_1, val_2, duration);
        case 'easeInCubic':
            return easeInCubic(ini_t, val_1, val_2, duration);
        case 'easeOutCubic':
            return easeOutCubic(ini_t, val_1, val_2, duration);
        case 'easeInOutCubic':
            return easeInOutCubic(ini_t, val_1, val_2, duration);
        case 'easeInQuart':
            return easeInQuart(ini_t, val_1, val_2, duration);
        case 'easeOutQuart':
            return easeOutQuart(ini_t, val_1, val_2, duration);
        case 'easeInOutQuart':
            return easeInOutQuart(ini_t, val_1, val_2, duration);
        case 'easeInQuint':
            return easeInQuint(ini_t, val_1, val_2, duration);
        case 'easeOutQuint':
            return easeOutQuint(ini_t, val_1, val_2, duration);
        case 'easeInOutQuint':
            return easeInOutQuint(ini_t, val_1, val_2, duration);
        case 'easeInElastic':
            return easeInElastic(ini_t, val_1, val_2, duration);
        case 'easeOutElastic':
            return easeOutElastic(ini_t, val_1, val_2, duration);
        case 'easeInOutElastic':
            return easeInOutElastic(ini_t, val_1, val_2, duration);
        case 'easeInBack':
            return easeInBack(ini_t, val_1, val_2, duration);
        case 'easeOutBack':
            return easeOutBack(ini_t, val_1, val_2, duration);
        case 'easeInOutBack':
            return easeInOutBack(ini_t, val_1, val_2, duration);
        case 'easeInBounce':
            return easeInBounce(ini_t, val_1, val_2, duration);
        case 'easeOutBounce':
            return easeOutBounce(ini_t, val_1, val_2, duration);
        case 'easeInOutBounce':
            return easeInOutBounce(ini_t, val_1, val_2, duration);
        default:
            return null;
    }
}


//Main Class
class Animator {
    constructor(runtime) {
        this.runtime = runtime
        this._formatMessage = runtime.getFormatMessage({
            'zh-cn': {
                'Ani.extensionName': 'Animator',
                'Ani.Animate': '以[menu]方式计算[ini_t]秒时[val_1]到[val_2]的值，持续时间为[duration]秒',//block
                'Ani.Move': '以[menu]方式移到[ini_t]秒时从[x_1][y_1]到[x_2][y_2]，持续时间[duration]秒',//block
                'Ani.easeInQuad': '二次缓入',
                'Ani.easeOutQuad': '二次缓出',
                'Ani.easeInOutQuad': '二次缓动',
                'Ani.easeInSine': '正弦缓入',
                'Ani.easeOutSine': '正弦缓出',
                'Ani.easeInOutSine': '正弦缓动',
                'Ani.easeInExpo': '指数缓入',
                'Ani.easeOutExpo': '指数缓出',
                'Ani.easeInOutExpo': '指数缓动',
                'Ani.easeInCirc': '圆形缓入',
                'Ani.easeOutCirc': '圆形缓出',
                'Ani.easeInOutCirc': '圆形缓动',
                'Ani.easeInCubic': '三次缓入',
                'Ani.easeOutCubic': '三次缓出',
                'Ani.easeInOutCubic': '三次缓动',
                'Ani.easeInQuart': '四次缓入',
                'Ani.easeOutQuart': '四次缓出',
                'Ani.easeInOutQuart': '四次缓动',
                'Ani.easeInQuint': '五次缓入',
                'Ani.easeOutQuint': '五次缓出',
                'Ani.easeInOutQuint': '五次缓动',
                'Ani.easeInElastic': '弹性缓入',
                'Ani.easeOutElastic': '弹性缓出',
                'Ani.easeInOutElastic': '弹性缓动',
                'Ani.easeInBack': '回弹缓入',
                'Ani.easeOutBack': '回弹缓出',
                'Ani.easeInOutBack': '回弹缓动',
                'Ani.easeInBounce': '弹跳缓入',
                'Ani.easeOutBounce': '弹跳缓出',
                'Ani.easeInOutBounce': '弹跳缓动',

                'Ani.getEffect': '获取特效[Effect]的值',//block
                'Ani.color': '颜色',
                'Ani.fisheye': '鱼眼',
                'Ani.whirl': '旋涡',
                'Ani.pixelate': '像素化',
                'Ani.mosaic': '马赛克',
                'Ani.brightness': '亮度',
                'Ani.ghost': '虚像',

                'Ani.docs': '📖文档',

                'Ani.Fac_ani': '🌟缓动运算',
                'Ani.Fac_dat': '🪄特效参数',
            },

            en: {
                'Ani.extensionName': "Animator",
                'Ani.Animate': 'Calculate the value between[val_1]and[val_2]using the[menu]method, with a duration of[duration]starting at[ini_t]',//block
                'Ani.Move': 'Move from[x_1][y_1]to[x_2][y_2]using the[menu]method, with a duration of[duration]starting at[ini_t]',//block
                'Ani.easeInQuad': 'Quadratic ease-in',
                'Ani.easeOutQuad': 'Quadratic ease-out',
                'Ani.easeInOutQuad': 'Quadratic ease-in-out',
                'Ani.easeInSine': 'Sine ease-in',
                'Ani.easeOutSine': 'Sine ease-out',
                'Ani.easeInOutSine': 'Sine ease-in-out',
                'Ani.easeInExpo': 'Exponential ease-in',
                'Ani.easeOutExpo': 'Exponential ease-out',
                'Ani.easeInOutExpo': 'Exponential ease-in-out',
                'Ani.easeInCirc': 'Circular ease-in',
                'Ani.easeOutCirc': 'Circular ease-out',
                'Ani.easeInOutCirc': 'Circular ease-in-out',
                'Ani.easeInCubic': 'Cubic ease-in',
                'Ani.easeOutCubic': 'Cubic ease-out',
                'Ani.easeInOutCubic': 'Cubic ease-in-out',
                'Ani.easeInQuart': 'Quartic ease-in',
                'Ani.easeOutQuart': 'Quartic ease-out',
                'Ani.easeInOutQuart': 'Quartic ease-in-out',
                'Ani.easeInQuint': 'Quintic ease-in',
                'Ani.easeOutQuint': 'Quintic ease-out',
                'Ani.easeInOutQuint': 'Quintic ease-in-out',
                'Ani.easeInElastic': 'Elastic ease-in',
                'Ani.easeOutElastic': 'Elastic ease-out',
                'Ani.easeInOutElastic': 'Elastic ease-in-out',
                'Ani.easeInBack': 'Back ease-in',
                'Ani.easeOutBack': 'Back ease-out',
                'Ani.easeInOutBack': 'Back ease-in-out',
                'Ani.easeInBounce': 'Bounce ease-in',
                'Ani.easeOutBounce': 'Bounce ease-out',
                'Ani.easeInOutBounce': 'Bounce ease-in-out',

                'Ani.getEffect': 'Get[Effect]value',//block
                'Ani.color': 'color',
                'Ani.fisheye': 'fisheye',
                'Ani.whirl': 'whirl',
                'Ani.pixelate': 'pixelate',
                'Ani.mosaic': 'mosaic',
                'Ani.brightness': 'brightness',
                'Ani.ghost': 'ghost',

                'Ani.docs': '📖Docs(English missing)',

                'Ani.Fac_ani': '🌟Easing functions',
                'Ani.Fac_dat': '🪄Effect parameters',
            },
        })
    }

    formatMessage(id) {
        return this._formatMessage({
            id,
            default: id,
            description: id,
        })
    }

    getInfo() {
        return {
            id: 'Animator', // ext id
            name: this.formatMessage('Ani.extensionName'), // ext name
            color1: '#023020',
            menuIconURI: ani_icon,
            blockIconURI: ani_icon,
            blocks: [
                {
                    blockType: "button",
                    text: this.formatMessage('Ani.docs'),
                    onClick: this.docs,
                },
                "---" + this.formatMessage("Ani.Fac_ani"),
                {
                    opcode: 'Animate',
                    blockType: 'reporter',
                    text: this.formatMessage('Ani.Animate'),
                    arguments: {
                        menu: {
                            type: "string",
                            menu: 'Ani',
                        },
                        ini_t: {
                            type: "number"
                        },
                        val_1: {
                            type: "number",
                        },
                        val_2: {
                            type: "number",
                        },
                        duration: {
                            type: "number",
                        }
                    }
                },
                {
                    opcode: 'Move',
                    blockType: 'command',
                    text: this.formatMessage('Ani.Move'),
                    arguments: {
                        menu: {
                            type: "string",
                            menu: 'Ani',
                        },
                        ini_t: {
                            type: "number",
                        },
                        x_1: {
                            type: "number",
                        },
                        x_2: {
                            type: "number",
                        },
                        y_1: {
                            type: "number",
                        },
                        y_2: {
                            type: "number",
                        },
                        duration: {
                            type: "number",
                        }
                    }
                },
                "---" + this.formatMessage("Ani.Fac_dat"),
                {
                    opcode: 'getEffect',
                    blockType: 'reporter',
                    text: this.formatMessage('Ani.getEffect'),
                    arguments: {
                        Effect: {
                            type: "string",
                            menu: 'Effect',
                        }
                    },
                    disableMonitor: true,//disable the tick before this block (bugs may accure when monitoring)
                },
            ],
            menus: {
                Ani: [
                    {
                        text: this.formatMessage('Ani.easeInQuad'),
                        value: 'easeInQuad'
                    },
                    {
                        text: this.formatMessage('Ani.easeOutQuad'),
                        value: 'easeOutQuad'
                    },
                    {
                        text: this.formatMessage('Ani.easeInOutQuad'),
                        value: 'easeInOutQuad'
                    },
                    {
                        text: this.formatMessage('Ani.easeInSine'),
                        value: 'easeInSine'
                    },
                    {
                        text: this.formatMessage('Ani.easeOutSine'),
                        value: 'easeOutSine'
                    },
                    {
                        text: this.formatMessage('Ani.easeInOutSine'),
                        value: 'easeInOutSine'
                    },
                    {
                        text: this.formatMessage('Ani.easeInExpo'),
                        value: 'easeInExpo'
                    },
                    {
                        text: this.formatMessage('Ani.easeOutExpo'),
                        value: 'easeOutExpo'
                    },
                    {
                        text: this.formatMessage('Ani.easeInOutExpo'),
                        value: 'easeInOutExpo'
                    },
                    {
                        text: this.formatMessage('Ani.easeInCirc'),
                        value: 'easeInCirc'
                    },
                    {
                        text: this.formatMessage('Ani.easeOutCirc'),
                        value: 'easeOutCirc'
                    },
                    {
                        text: this.formatMessage('Ani.easeInOutCirc'),
                        value: 'easeInOutCirc'
                    },
                    {
                        text: this.formatMessage('Ani.easeInCubic'),
                        value: 'easeInCubic'
                    },
                    {
                        text: this.formatMessage('Ani.easeOutCubic'),
                        value: 'easeOutCubic'
                    },
                    {
                        text: this.formatMessage('Ani.easeInOutCubic'),
                        value: 'easeInOutCubic'
                    },
                    {
                        text: this.formatMessage('Ani.easeInQuart'),
                        value: 'easeInQuart'
                    },
                    {
                        text: this.formatMessage('Ani.easeOutQuart'),
                        value: 'easeOutQuart'
                    },
                    {
                        text: this.formatMessage('Ani.easeInOutQuart'),
                        value: 'easeInOutQuart'
                    },
                    {
                        text: this.formatMessage('Ani.easeInQuint'),
                        value: 'easeInQuint'
                    },
                    {
                        text: this.formatMessage('Ani.easeOutQuint'),
                        value: 'easeOutQuint'
                    },
                    {
                        text: this.formatMessage('Ani.easeInOutQuint'),
                        value: 'easeInOutQuint'
                    },
                    {
                        text: this.formatMessage('Ani.easeInElastic'),
                        value: 'easeInElastic'
                    },
                    {
                        text: this.formatMessage('Ani.easeOutElastic'),
                        value: 'easeOutElastic'
                    },
                    {
                        text: this.formatMessage('Ani.easeInOutElastic'),
                        value: 'easeInOutElastic'
                    },
                    {
                        text: this.formatMessage('Ani.easeInBack'),
                        value: 'easeInBack'
                    },
                    {
                        text: this.formatMessage('Ani.easeOutBack'),
                        value: 'easeOutBack'
                    },
                    {
                        text: this.formatMessage('Ani.easeInOutBack'),
                        value: 'easeInOutBack'
                    },
                    {
                        text: this.formatMessage('Ani.easeInBounce'),
                        value: 'easeInBounce'
                    },
                    {
                        text: this.formatMessage('Ani.easeOutBounce'),
                        value: 'easeOutBounce'
                    },
                    {
                        text: this.formatMessage('Ani.easeInOutBounce'),
                        value: 'easeInOutBounce'
                    }
                ],
                Effect: [
                    {
                        text: this.formatMessage('Ani.color'),
                        value: 'color'
                    },
                    {
                        text: this.formatMessage('Ani.fisheye'),
                        value: 'fisheye'
                    },
                    {
                        text: this.formatMessage('Ani.whirl'),
                        value: 'whirl'
                    },
                    {
                        text: this.formatMessage('Ani.pixelate'),
                        value: 'pixelate'
                    },
                    {
                        text: this.formatMessage('Ani.mosaic'),
                        value: 'mosaic'
                    },
                    {
                        text: this.formatMessage('Ani.brightness'),
                        value: 'brightness'
                    },
                    {
                        text: this.formatMessage('Ani.ghost'),
                        value: 'ghost'
                    },
                ]
            }
        }
    }
    //Defines start here

    docs() {
        console.log("Opening Animator.js documentation in new window...");
        window.open("https://gandi.rth1.app/Docs/Animator.js.html");
    }

    //value return
    //note that for advanced animation involving movements, resizing, effects, or directions, this block would be better than the Move() function.
    Animate(args) {
        //fix input values (a logic related bug, but this can fix it)
        let val_2 = args.val_2 - args.val_1

        return Ani_Tweaks(args.menu, args.ini_t, args.val_1, val_2, args.duration);//val_2 not args.val2 is ment to fix the logic bug mentioned before.
    }

    //move (frame animation)
    Move(args, util) {
        let x_2 = args.x_2 - args.x_1;
        let y_2 = args.y_2 - args.y_1;

        util.target.setXY(Ani_Tweaks(args.menu, args.ini_t, args.x_1, x_2, args.duration), Ani_Tweaks(args.menu, args.ini_t, args.y_1, y_2, args.duration));
    }

    //getEffect
    //note that this function can be replaced by Arkos' Extensions (@Arkos). It would be easier to use these blocks here compared to finding it in Arkos' Extensions.
    getEffect(args, util) {
        return util.target.effects[args.Effect];
    }
}


window.tempExt = {
    Extension: Animator,
    info: {
        name: 'gfx.extensionName',
        description: 'gfx.description',
        extensionId: 'gfxajAnimator',
        iconURL: ani_cover,
        insetIconURL: ani_icon,
        featured: true,
        disabled: false,
        collaborator: '@官方小傲娇',
    },
    l10n: {
        'zh-cn': {
            'gfx.extensionName': 'Animator',
            'gfx.description': '让你的角色动起来🏃‍♂️',
        },
        en: {
            'gfx.extensionName': 'Animator',
            'gfx.description': 'Make your characters move🏃‍♂️',
        },
    },
}


/*
Reference(s):
https://easings.net/
https://spicyyoghurt.com/tools/easing-functions
*/
