...
class RenderTheWorld {
    constructor(runtime) {
        ...
        this._formatMessage = runtime.getFormatMessage({
            "zh-cn": {
                ...
                "RenderTheWorld.importGLTF": "导入或重置GLTF模型: [name] GLTF模型网址: [gltfurl] 位置: x[x] y[y] z[z] [YN]投射阴影 [YN2]被投射阴影",
            ...
            en: {
                ...
                "RenderTheWorld.importGLTF": "reset or make a GLTF Model: [name] GLTF url: [gltfurl] position: x[x] y[y] z[z] [YN]cast shadows [YN2]shadow cast",
    ...
    init(args) {
        ...
        // 创建threejs显示canvas
        //this._ccw = document.getElementsByClassName('gandi_stage_stage_1fD7k')[0].getElementsByTagName("canvas")[0];
        if (this.scratch_canvas.parentElement.getElementsByClassName("RenderTheWorld").length == 0) {
    ...
    makeCube(args) {
        ...
        // 长方体
        let geometry = new THREE.BoxGeometry (Scratch.Cast.toNumber(args.a), Scratch.Cast.toNumber(args.b), Scratch.Cast.toNumber(args.h));
    ...
    makeSphere(args) {
        ...
        let name = Scratch.Cast.toString(args.name)
        // 长方体
        let geometry = new THREE.SphereGeometry(Scratch.Cast.toNumber(args.radius), Scratch.Cast.toNumber(args.w), Scratch.Cast.toNumber(args.h));
        // let material = new THREE.MeshPhongMaterial({
        //     color: Scratch.Cast.toNumber(args.color),
        // });
        // 纹理
    ...
    makePlane(args) {
        ...
        let name = Scratch.Cast.toString(args.name)
        // 长方体
        let geometry = new THREE.PlaneGeometry(Scratch.Cast.toNumber(args.a), Scratch.Cast.toNumber(args.b));
        // let material = new THREE.MeshPhongMaterial({
        //     color: Scratch.Cast.toNumber(args.color),
        // });
        // 纹理
    ...
    rotationObject(args) {
        ...
        if (name in this.objects) {
            // 设置旋转角度
            this.objects[name].rotation.set(THREE.MathUtils.degToRad(Scratch.Cast.toNumber(args.x)), THREE.MathUtils.degToRad(Scratch.Cast.toNumber(args.y)), THREE.MathUtils.degToRad(Scratch.Cast.toNumber(args.z)));
        } else {
            return ;
        }
    }
    ...
    scaleObject(args) {
        ...
        if (name in this.objects) {
            // 设置缩放
            this.objects[name].scale.set(Scratch.Cast.toNumber(args.x), Scratch.Cast.toNumber(args.y), Scratch.Cast.toNumber(args.z))
        } else {
            return ;
        }
    }
