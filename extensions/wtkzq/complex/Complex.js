...
class Complex {
    constructor(runtime) {
        ...
        this._formatMessage = runtime.getFormatMessage({
          'zh-cn': {
            ...
            'complex.sqrt': '平方根',
            'complex.convert_abj': '将[A]转换为a+bj形式'
          },
          ...
          en: {
            ...
            'complex.sqrt': 'sqrt',
            'complex.convert_abj': 'convert [A] to a+bj'
          },
        })
      }
        ...
        return {
            ...
            blocks: [
                ...
                },
                {
                    opcode: 'convert_abj',
                    blockType: Scratch.BlockType.REPORTER,
                    text: this.formatMessage('complex.convert_abj'),
                    arguments: {
                        A: {
                            type: Scratch.ArgumentType.STRING
                        }
    ...
    div({ A, B }) {
        ...
        A = this._to_complex(A)
        B = this._to_complex(B)
        return this._to_comma([(A[0] * B[0] + A[1] * B[1]) / (B[0] * B[0] + B[1] * B[1]),
    ...
    func({ FUNC, A }) {
        ...
        if (FUNC === 'abs')
            ...
            return this._to_comma(A.map(Math.floor))
        else if (FUNC === 'ceiling')
            return this._to_comma(A.map(Math.ceil))
        else if (FUNC === 'sqrt')
            if (A[1] == 0)
                if (A[0] >= 0)
                    return this._to_comma([Math.sqrt(A[0]), 0])
                else
                    return this._to_comma([0, Math.sqrt(-A[0])])
            else return NaN
    }
