...
class ScopeVar {
  constructor(runtime) {
    ...
    this.initFormatMessage({
      ...
      en: {
        ...
        urlButton: "📖 Tutorial",
        "tip.compiler": "⚠️ May not work in compile mode(See documentation)",
        url: "https://learn.ccw.site/article/49da22df-a178-4cce-86c7-366df75f7d75",
  ...
  forEachWithList({ LIST, VAR, I }, util) {
    ...
    if (typeof stackFrame.loopCounter === "undefined") {
      ...
      if (typeof LIST === "object") {
        ...
        if (this.runtime.SafeObject) {
          list = this.runtime.SafeObject.getActualObject(list);
        }
        ...
        // 根据Id或列表名查找，没找到则返回
        list = util.target.lookupVariableById(LIST);
        if (!list) {
    ...
    if (type === "obj") {
      ...
      vars[stackFrame.varName] = list[prop] ?? "";
    } else {
      vars[stackFrame.idxName] = idx + 1;
      vars[stackFrame.varName] = list[idx] ?? "";
    }
    util.startBranch(1, true);
  }
}
